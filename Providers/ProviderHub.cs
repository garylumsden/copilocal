...
internal sealed class ProviderHub(IProcessRunner proc, IHttpGateway http)
{
    ...
    List<MenuItem> GatherFoundry()
    {
        ...
        if (code != 0) return items;
        foreach (var (id, loadId, tools) in ProviderParsers.ParseFoundry(outp))
    ...
    List<MenuItem> GatherLiteLlm()
    {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                ...
                foreach (var id in ProviderParsers.ParseLiteLlmModels(body))
        ...
        try
        {
            ...
            foreach (var el in models.EnumerateArray())
            {
                ...
                if (el.TryGetProperty("name", out var n) && n.ValueKind == JsonValueKind.String
                    && n.GetString() == model) return ProviderParsers.NumOrZero(el, "context_length");
            }
            return 0;
        }
    ...
    int FoundryContextLength(MenuItem m)
    {
        try
        {
            ...
            return d.RootElement.ValueKind == JsonValueKind.Object
                ...
                ? ProviderParsers.NumOrZero(model, "contextLength") : 0;
        }
        catch (JsonException)
        {
    ...
    internal (WarmStatus Status, string Detail, bool Reasoning) WarmUp(string baseUrl, string model, string? bearerToken = null)
    {
        try
        {
            ...
            if (!ok)
                return (WarmStatus.Failed, ProviderResponses.HttpFailureDetail(status, body), false);
            ...
            bool reasoning = !string.IsNullOrWhiteSpace(ProviderResponses.ExtractMessageField(body, "reasoning"))
                          || !string.IsNullOrWhiteSpace(ProviderResponses.ExtractMessageField(body, "reasoning_content"));
            string text = ProviderResponses.ExtractMessageField(body, "content");
            ...
            if (ProviderResponses.LooksGarbled(text))
                return (WarmStatus.Suspect, $"output looks garbled: \"{ProviderResponses.Trim(text)}\"", reasoning);
            return (WarmStatus.Ok, ProviderResponses.Trim(text), reasoning);
        }
    ...
    internal (ToolStatus Status, string Detail, bool Reasoning) ProbeToolCalling(string baseUrl, string model, string? bearerToken = null)
    {
        try
        {
            ...
            var (ok, status, body) = http.PostJson($"{baseUrl}/chat/completions", payload, 120_000, bearerToken);
            if (!ok) return (ToolStatus.Inconclusive, ProviderResponses.HttpFailureDetail(status, body), false);
            bool reasoning = !string.IsNullOrWhiteSpace(ProviderResponses.ExtractMessageField(body, "reasoning"))
                          || !string.IsNullOrWhiteSpace(ProviderResponses.ExtractMessageField(body, "reasoning_content"));
            if (ProviderResponses.HasToolCall(body)) return (ToolStatus.Ok, "native tool calling OK", reasoning);
            if (ProviderResponses.LooksLikeToolCallText(ProviderResponses.ExtractMessageField(body, "content")))
